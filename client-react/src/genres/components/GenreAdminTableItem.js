...
const GENRES_ADMIN_URL = "/api/admin/genres/";

const GenreAdminTableItem = (props) => {
  const { auth } = useAuth();
  const [confirmedRequest, setConfirmedRequest] = useState(false);

  const deleteHandler = async () => {
    try {
      await axios.delete(GENRES_ADMIN_URL + props.item.id, {
        headers: {
...
export default GenreAdminTableItem;
